...
public class ProcessTestMyProcess {
	...
	private String filename = "C:\\Users\\Jimmy\\eclipse-workspace\\BulkBuyer\\src\\main\\resources\\diagrams\\BulkBuyer.bpmn";
	...
	public void startProcess() throws Exception {
		...
		WfXML wf = new WfXML(10000);
		
		System.out.println("BB process started at " + new Date().toString());
		wf.CreateInstance();  //向Broker发送指令，通知其部署合约 
		
		Robot r = new Robot(); 
        r.delay(20000);   
        
        wf.ChangeState("call BB_Manuf_sendOrder()");  //向Broker发送指令，通知其调用函数
        
        wf.StateChanged();  //接收来自Broker的状态改变信息，对应receive start
        
        wf.StateChanged();  //接收来自Broker的状态改变信息，对应receive product
        
        //流程结束
        System.out.println("BB process finished at " + new Date().toString());
        //wf.StateChanged();  //阻塞，连接不断开
