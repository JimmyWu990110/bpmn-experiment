...
public class ProcessTestMyProcess {
	...
	private String filename = "C:\\Users\\Jimmy\\eclipse-workspace\\Manufacturer\\src\\main\\resources\\diagrams\\Manufacturer.bpmn";
	...
	public void startProcess() throws Exception {
		...
		WfXML wf = new WfXML(11000);
		
		System.out.println("Manuf process started at " + new Date().toString());
		wf.receiveAddress();  //接收合约地址
		
		wf.StateChanged();  //接收来自Broker的状态改变信息，对应receive order A
		
		//执行任务CalculateDemand
		Task CalculateDemand = activitiRule.getTaskService().createTaskQuery().processInstanceId(processInstance.getId()).singleResult();
		Robot r = new Robot(); 
        r.delay(10000);   
        System.out.println("Task CalculateDemand finished at "+ new Date().toString());
		
		wf.ChangeState("call Manuf_Mid_placeOrder()");  //向Broker发送指令，通知其调用函数
		
		wf.StateChanged();  //接收来自Broker的状态改变信息，对应receive order B
		r.delay(1000); 
		
		wf.ChangeState("call Manuf_BB_reportStart()");  //向Broker发送指令，通知其调用函数
		
		//执行任务Produce
		Task Produce = activitiRule.getTaskService().createTaskQuery().processInstanceId(processInstance.getId()).singleResult();
		r.delay(10000); 
		System.out.println("Task Produce finished at "+ new Date().toString());
		
		wf.ChangeState("call Manuf_BB_deliverProduct()");  //向Broker发送指令，通知其调用函数
		
        //流程结束
    	System.out.println("Manuf process finished at " + new Date().toString());
	}
}