...
public class ProcessTestMyProcess {
	...
	private String filename = "C:\\Users\\Jimmy\\eclipse-workspace\\Supplier\\src\\main\\resources\\diagrams\\Supplier.bpmn";
	...
	public void startProcess() throws Exception {
		...
		WfXML wf = new WfXML(13000);
		
		System.out.println("Sup process started at " + new Date().toString());
		...
		//执行任务PrepareTransport
		Task PrepareTransport = activitiRule.getTaskService().createTaskQuery().processInstanceId(processInstance.getId()).singleResult();
		r.delay(10000);   
		System.out.println("Task PrepareTransport finished at "+ new Date().toString());
		
		wf.StateChanged();  //接收来自Broker的状态改变信息，对应receive request
		r.delay(4000);
		
		wf.ChangeState("call Sup_SC_provideDetails()");  //向Broker发送指令，通知其调用函数
	    r.delay(5000);
	        
	    wf.ChangeState("call Sup_SC_provideWaybill()");  //向Broker发送指令，通知其调用函数
	    
	    //流程结束
		System.out.println("Sup process finished at " + new Date().toString());
