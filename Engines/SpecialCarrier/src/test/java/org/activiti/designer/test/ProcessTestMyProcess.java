...
public class ProcessTestMyProcess {
	...
	private String filename = "C:\\Users\\Jimmy\\eclipse-workspace\\SpecialCarrier\\src\\main\\resources\\diagrams\\SpecialCarrier.bpmn";
	...
	public void startProcess() throws Exception {
		...
		WfXML wf = new WfXML(14000);
		
		System.out.println("SC process started at " + new Date().toString());
		...
		wf.ChangeState("call SC_Sup_requestDetails()");  //向Broker发送指令，通知其调用函数
		
		wf.StateChanged();  //接收来自Broker的状态改变信息，对应receive details
		
		wf.StateChanged();  //接收来自Broker的状态改变信息，对应receive waybill
		r.delay(2000); 
		
		wf.ChangeState("call SC_Manuf_deliverOrder()");  //向Broker发送指令，通知其调用函数
		
		//流程结束
		System.out.println("SC process finished at " + new Date().toString());
