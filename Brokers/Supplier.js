...
// 打印账户信息
console.log('account: ' + web3.eth.accounts[3]);
...
servertoEngine.on('listening', function() {  //服务器不能重复listen一个端口，要放在外面
	console.log('listen to Sup Engine: ' + servertoEngine.address().port);
});

servertoEngine.on('connection', function (Enginesocket) {
	console.log('connect to Sup Engine success, contract address: ' + contractAddr);
	...
	var events = myInstance.allEvents(function(error, log){
		if (!error) {
			//console.log(log);
			if (log.args.receiver == 'Supplier') {
                console.log(log);
				console.log('send ' + log.args.event_ + ' to Sup Engine');
	...
	Enginesocket.on('data', function (data) { //接收到数据的时候触发
		...
		if (jobj.instruction == 'call Sup_SC_provideDetails()') { 
			myInstance.Sup_SC_provideDetails.sendTransaction({
				from: web3.eth.accounts[3],
				gas: 3141592
			});
		}
		if (jobj.instruction == 'call Sup_SC_provideWaybill()') { 
			myInstance.Sup_SC_provideWaybill.sendTransaction({
				from: web3.eth.accounts[3],
				gas: 3141592
			});
		}
	...
	Enginesocket.on('close', function(data) { // client关闭
		console.log('connection to Sup Engine closed');
	});
...
server.on('connection', function (socket) { //当一个新的连接建立时触发，可接收一个socket对象
    ...
    socket.on('data',function (data) {
		...
		setTimeout(function(){
			socket.write('Sup Broker: contract address recieved');
		}, 4000);	
