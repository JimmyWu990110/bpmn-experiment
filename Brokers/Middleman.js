...
servertoEngine.on('listening', function() {  //服务器不能重复listen一个端口，要放在外面
	console.log('listen to Mid Engine: ' + servertoEngine.address().port);
});

servertoEngine.on('connection', function (Enginesocket) {
	console.log('connect to Mid Engine success, contract address: ' + contractAddr);
	...
	var events = myInstance.allEvents(function(error, log){
		if (!error) {
			//console.log(log);
			if (log.args.receiver == 'Middleman') {
				console.log(log);
				console.log('send ' + log.args.event_ + ' to Mid Engine');
	...
	Enginesocket.on('data',function (data) { //接收到数据的时候触发
		...
		if (jobj.instruction == 'call Mid_Sup_forwardOrder()') { 
			myInstance.Mid_Sup_forwardOrder.sendTransaction({
				from: web3.eth.accounts[2],
				gas: 3141592
			});
		}
		if (jobj.instruction == 'call Mid_SC_orderTransport()') { 
			myInstance.Mid_SC_orderTransport.sendTransaction({
				from: web3.eth.accounts[2],
				gas: 3141592
			});
		}
	...
	Enginesocket.on('close', function(data) { // client关闭
		console.log('connection to Mid Engine closed');
	});
...
server.on('connection', function (socket) { //当一个新的连接建立时触发，可接收一个socket对象
    ...
    socket.on('data',function (data) {
		...
		setTimeout(function(){
			socket.write('Mid Broker: contract address recieved');
		}, 3000);
