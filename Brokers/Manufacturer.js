...
// 打印账户信息
console.log('account: ' + web3.eth.accounts[1]);
...
servertoEngine.on('listening', function() {  //服务器不能重复listen一个端口，要放在外面
	console.log('listen to Manuf Engine: ' + servertoEngine.address().port);
});
	
servertoEngine.on('connection', function (Enginesocket) {  //连接Manuf引擎，直到流程结束断开
	console.log('connect to Manuf Engine success, contract address: ' + contractAddr);
	...
	var events = myInstance.allEvents(function(error, log){
		if (!error) {
			//console.log(log);
			if (log.args.receiver == 'Manufacturer') {
				console.log(log);
				console.log('send ' + log.args.event_ + ' to Manuf Engine');
	...
	Enginesocket.on('data',function (data) { //接收到数据的时候触发
		...
		if (jobj.instruction == 'call Manuf_Mid_placeOrder()') { 
			myInstance.Manuf_Mid_placeOrder.sendTransaction({
				from: web3.eth.accounts[1],
				gas: 3141592
			});
		}
		if (jobj.instruction == 'call Manuf_BB_reportStart()') { 
			myInstance.Manuf_BB_reportStart.sendTransaction({
				from: web3.eth.accounts[1],
				gas: 3141592
			});
		}
		if (jobj.instruction == 'call Manuf_BB_deliverProduct()') { 
			myInstance.Manuf_BB_deliverProduct.sendTransaction({
				from: web3.eth.accounts[1],
				gas: 3141592
			});
		}
	...
	Enginesocket.on('close', function(data) { // client关闭
		console.log('connection to Manuf Engine closed');
	});
...
server.on('connection', function (socket) { //当一个新的连接建立时触发，可接收一个socket对象
    ...
    socket.on('data',function (data) {
		...
		setTimeout(function(){
			socket.write('Manuf Broker: contract address recieved');
		}, 2000);
